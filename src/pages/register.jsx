        ...
        <div className='bg-zinc-950 font-mono h-screen w-screen flex justify-center items-center'>
            <form autoComplete='off' onSubmit={handleSubmit} className='text-white max-h-max h-3/5 w-2/5 bg-zinc-800 flex flex-col justify-center items-center gap-4'>
                ...
                <div className='flex flex-col justify-center w-3/5 max-w-xl content-center'>
                    ...
                    <input aria-invalid={validUsername ? false : true} aria-describedby='userDescription' onBlur={() => setUserFocus(false)} onFocus={() => setUserFocus(true)} onChange={(e) => setUsername(e.target.value)} required className='text-black duration-100 hover:brightness-90 rounded-md h-8 text-lg' id='username' type="text" />
                    ...
                    <input onChange={(e) => setPassword(e.target.value)} onBlur={() => setPasswordFocus(false)} onFocus={() => setPasswordFocus(true)} required className={!passwordFocus && !validPassword && password ? 'text-black duration-100 hover:brightness-90 rounded-md h-8 text-lg border-red-500 border-2' : 'text-black duration-100 hover:brightness-90 rounded-md h-8 text-lg'} id='password' aria-describedby='passwordDescription' type="password" />
                ...
                <p className='text-emerald-500' >Já registrado? <a href="./login" className='underline text-blue-100'>login</a></p>
            </form>
        </div>
    )
}